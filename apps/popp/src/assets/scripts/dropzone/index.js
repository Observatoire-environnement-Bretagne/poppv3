                ...
                if (data.exif){
                    ...
                    if(data.exif.getAll().Exif){
                        if(data.exif.getAll().Exif.DateTimeOriginal){
                            ...
                            var dayField = dateTime.getFullYear() + "-" + ("0" + (dateTime.getMonth() + 1) ).slice(-2) + "-" + ("0" + dateTime.getDate()).slice(-2);
...
Dropzone.options.importPhotoForm = {
    ...
    paramName: "file", // The name that will be used to transfer the file
    dictRemoveFile: "Supprimer",
    dictDefaultMessage: "Déposer le fichier ici (poids inférieur à 256Mo), les formats acceptés sont .png, .gif, .jpg, .jpeg",
    maxFiles: 1,
    addRemoveLinks: true,
    maxFilesize: 256, //MB
