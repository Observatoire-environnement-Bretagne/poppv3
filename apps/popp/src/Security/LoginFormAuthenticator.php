...
class LoginFormAuthenticator extends AbstractFormLoginAuthenticator
{
    ...
    public function onAuthenticationSuccess(Request $request, TokenInterface $token, $providerKey)
    {
        ...
        //$targetPath = $this->router->generate('admin_page'); // default redirect path.

        $user = $token->getUser();
        if ($user->getDatederncnx() !== null) {
            $dateTime = new \DateTime();
            $user->setDatederncnx($dateTime);

            $this->entityManager->flush();

        }else {
            $firstCnx = true;
            // S'il s'agit de la 1ère connexion alors on redirige vers la page de changement de mot de passe
        ...
        // redirect to some "app_homepage" route - of wherever you want
        return new RedirectResponse($this->urlGenerator->generate('public_page'));
    }
